...
class MexcConnector(BaseExchangeConnector):
    ...
    async def parse_message(self, raw_message, symbols=None) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            # Handle text (JSON) messages - errors, confirmations, pongs
            if isinstance(raw_message, str):
                try:
                    ...
                    # Handle other JSON messages (confirmations, etc.)
    ...
    async def _handle_messages(self, websocket, symbols: List[str]):
        """Handle incoming WebSocket messages with MEXC-specific symbol passing"""
        try:
            async for message in websocket:
                ...
                try:
                    # Parse message using MEXC-specific implementation with symbols list
                    ...
                    # Check if this is a subscription error that affects all symbols
                    if isinstance(message, str) and "error" in message.lower():
                        await self.report_error(symbols, "SubscriptionError", str(e))
                    else:
