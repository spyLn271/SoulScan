...
Raydium's AMM works in the same way as Uniswap V2 does.
Like in Raydium CLMM, there is no adaptive fee.
...
pub fn calculate_swap(x_reserves: u128, y_reserves: u128, delta_amount: u128, fee_rate: u32,
                      x_to_y: bool, amount_specified_is_in: bool) -> Result<SwapStepResult, SoulMathError> {
    ...
    if x_to_y && amount_specified_is_in {
        ...
        let total_amount_in = calculate_amount_with_fee(result.amount_in,
                                                              fee_rate,
                                                              false)?;
