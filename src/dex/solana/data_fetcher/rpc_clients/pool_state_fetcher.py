...
if TYPE_CHECKING:
    ...
    from src.dex.solana.orca.state.orca_clmm import OrcaCLMM
    from src.dex.solana.raydium.state.raydium_clmm import RaydiumCLMM
    from src.dex.solana.raydium.state.raydium_hybrid_amm import RaydiumHybridAMM
####################################
...
class PoolStateFetcher:
    ...
    async def _set_up_cache(self, provider_instance, metadata: dict, addresses: list) -> bool:
        raise pfs_exception.NoImplementationException("_set_up_cache not implemented.")

    async def _state_fetcher(self, provider_instance, metadata: dict, addresses: list) -> dict:
        raise pfs_exception.NoImplementationException("_state_fetcher not implemented.")
    ...
    async def main(self):
        ...
        async with self.provider(**self.provider_kwargs) as provider_instance:
            while True:
                ...
                except pfs_exception.NoMetadataException as e:
                    self.logger.error(f"No metadata found for {self.market} {self.version} ({e}).")
                    break
                except pfs_exception.NoImplementationException as e:
