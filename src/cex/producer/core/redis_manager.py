...
- Bounded max_connections per worker PROCESS (each spawned worker has its own pool)
...
class RedisManager:
    ...
    async def get_pool_info(cls) -> dict:
        ...
        # Public/stable attrs only; private internals differ across redis-py versions.
        try:
            stats = {"status": "active",
                     "max_connections": getattr(cls._pool, "max_connections", None)}
            in_use = getattr(cls._pool, "_in_use_connections", None)
            available = getattr(cls._pool, "_available_connections", None)
            if in_use is not None:
                stats["in_use_connections"] = len(in_use)
            if available is not None:
                stats["available_connections"] = len(available)
            return stats
        except Exception as e:
