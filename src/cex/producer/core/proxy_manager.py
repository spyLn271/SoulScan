...
class ProxyManager:
    ...
    def __init__(self, exchange_name: str, proxy_config: Dict[str, Any], proxy_list: Optional[List[Dict[str, Any]]] = None,
        ...
        # Total proxy rotations triggered by disconnects (exposed in heartbeat).
        self.rotations = 0
    ...
    async def rotate_on_disconnect(self, symbol: str = None):
        """Switch to a different proxy after a disconnect so the next reconnect
        does not reuse the just-failed route. Breaks any sticky mapping for the
        symbol and advances the rotation pointer. Persistent per-proxy demotion is
        still handled by handle_connection_failure()."""
        if self.mode == 'none' or not self.proxies:
            return
        if symbol is not None:
            self.symbol_proxy_mapping.pop(symbol, None)
            self.symbol_route_mapping_flex.pop(symbol, None)
        self.current_proxy_index = (self.current_proxy_index + 1) % len(self.proxies)
        if self.mode == 'flex' and self.flex_routes:
            self.current_route_index = (self.current_route_index + 1) % len(self.flex_routes)
        self.rotations += 1
        self.connection_stats['rotations'] = self.connection_stats.get('rotations', 0) + 1
        self.logger.info(
            f"Rotated proxy on disconnect (symbol={symbol}) -> index "
            f"{self.current_proxy_index}/{max(0, len(self.proxies) - 1)}, total={self.rotations}")

    def current_proxy_label(self) -> str:
        """Short label for the proxy the rotation pointer currently favours
        (for heartbeat/observability). 'direct' when no proxies are usable."""
        if self.mode == 'none' or not self.proxies:
            return 'direct'
