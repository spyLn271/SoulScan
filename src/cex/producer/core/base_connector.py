...
from src.cex.producer.config import (
    ...
    get_error_queue_key, ACCEPTABLE_QUOTE_ASSETS, MONITORING_CONFIG
)
...
class BaseExchangeConnector(ABC):
    ...
    async def _retry_redis_operation(self, operation, max_retries=3, base_delay=0.1):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Only retry on connection pool exhaustion errors
    ...
    async def _process_orderbook_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            # Store in Redis with maxlen=10 (keep only last 10 updates per symbol) with retry logic
            async def _do_xadd():
                await self.redis_client.xadd(
                    stream_key,
                    redis_data,
                    maxlen=10  # Keep last 10 updates per symbol
                )
    ...
    def _filter_symbols_for_worker(self, all_symbols: List[str]) -> List[str]:
        ...
        # DEBUG: Check if AXSUSDT is in this worker's list and at what index
        if 'AXSUSDT' in all_symbols:
            axs_index = all_symbols.index('AXSUSDT')
            axs_worker = axs_index % self.num_workers
            has_axs = 'AXSUSDT' in worker_symbols
            self.logger.warning(
                f"[DEBUG] AXSUSDT: index={axs_index}, assigned_worker={axs_worker}, "
                f"this_worker={self.worker_id}, has_it={has_axs}"
            )
    ...
    async def _monitor_symbol_changes(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                self.logger.error(f"Error in symbol change monitoring: {e}")
