...
class GateioConnector(BaseExchangeConnector):
    ...
    async def get_connection_kwargs(self, symbol: str = None) -> Dict[str, Any]:
        ...
        # Browser-like headers to avoid bot detection. NOTE: websockets 12.0 (pinned)
        # uses `extra_headers`; `additional_headers` is the 14.0+ name and raises
        # "create_connection() got an unexpected keyword argument" on 12.0 — which
        # silently broke EVERY gate.io connection until this was corrected.
