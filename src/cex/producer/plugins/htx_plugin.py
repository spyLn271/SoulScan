...
from src.cex.producer.config import MONITORING_CONFIG
...
class HtxConnector(BaseExchangeConnector):
    ...
    async def _handle_messages(self, websocket, symbols: List[str]):
        """Handle HTX messages with ping/pong AND stale-stream detection.

        HTX pings arrive as gzipped JSON detected inside parse_message (returns a
        {'type': 'ping_response'} dict), so this keeps a bespoke loop, but reads
        with a per-recv timeout so a dead socket is reconnected like every other
        exchange (was a blocking `async for` with no stale detection)."""
        ...
        try:
                ...
                try:
                    ...
                    if parsed_data:
                        if parsed_data.get('type') == 'ping_response':
                            ...
                            self.logger.debug("Sent pong response to HTX ping")
                        else:
            ...
            self.logger.info("HTX WebSocket connection closed")
            raise
