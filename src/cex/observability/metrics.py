#!/usr/bin/env python3
"""
Prometheus metrics for cex_v2. Optional: if prometheus_client is missing, every metric degrades to a
no-op so the producer still runs. Each process owns its own default registry and serves its own
/metrics endpoint (see start_metrics_server) — labeled by exchange/market so one scrape covers a
process's work. Order-book workers and the market-data manager each get a distinct port (assigned by
the supervisor / config), so Prometheus scrapes them as separate targets.
"""
import logging
...
try:
    ...
    HAVE_PROM = True
except ImportError:  # graceful fallback — cex_v2 runs without metrics
    HAVE_PROM = False
...
# --- market-data (label: exchange, market[, result]) ---
...
def start_metrics_server(port: int, enabled: bool = True, addr: str = "127.0.0.1") -> bool:
    """Start a /metrics HTTP server on addr:port. Returns True if serving. NEVER raises (graceful): a
    bind failure (port in use) or a missing prometheus_client just disables metrics for this process.
    Binds localhost by default so internals are not exposed on the public interface."""
