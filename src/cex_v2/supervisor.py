...
Run: python -m src.cex_v2.supervisor
"""
import asyncio
import importlib
import json
...
from src.cex_v2.config import EXCHANGES, MONITORING, METRICS, get_redis_config, get_heartbeat_key
from src.cex_v2.core.limits import raise_fd_limit, install_orphan_guards, acquire_singleton_lock
...
def _discover(markets=("spot", "futures"), exchanges=None) -> list:
    ...
    for exchange, cfg in EXCHANGES.items():
        ...
        for mt in ("spot", "futures"):
            ...
            mod = f"src.cex_v2.plugins.{exchange}"
            try:
