#!/usr/bin/env python3
"""
OrderBookConnector — the cex_v2 connector base (rebuilt from ORDERBOOK_PRODUCER_SPEC.md invariants).

Write path (the headline invariant): receive frame -> plugin.parse() -> self._latest[symbol]
(non-blocking) ; a flush task pipelines the latest per symbol every flush_interval. NO awaited
xadd in the receive loop (that capped throughput and built permanent lag). Serialization is done
once per symbol per flush, not per message. Collapse-to-latest drops stale snapshots (correct for a
snapshot feed). Plugins implement only exchange-specific parse/subscribe/url.

Optional perf: orjson if importable, else stdlib json.
"""
import abc
import asyncio
import logging
...
from src.cex_v2.core.ws_connect import connect_with_proxy_config
from src.cex_v2.observability import emit_perf
from src.cex_v2.observability import metrics as _m


# Parsed update from a plugin. bids/asks: [[price, qty], ...] (str or num); coerced at flush.
Book = namedtuple("Book", "symbol bids asks event_ts_ms")


class ResyncRequired(Exception):
    """A plugin signals an unrecoverable in-stream gap (e.g. a missed orderbook delta). The core
    treats it as a connection fault → reconnect + resubscribe → fresh snapshot. (Unlike a malformed
    frame, which the recv loop drops and continues past.)"""
...
def coerce_levels(levels) -> List[List[float]]:
    ...
    for lvl in levels:
        ...
        if not (math.isfinite(p) and math.isfinite(q)):
            continue  # drop nan/inf garbage — float() accepts them and orjson serializes them as null
        out.append([p, q])
    return out
...
class OrderBookConnector(abc.ABC):
    def __init__(self, exchange: str, market_type: str = "spot",
        ...
        self.ping_interval = mc.get("ping_interval")  # ms; None => no client app-ping
        ...
        self._absent_scans: Dict[str, int] = {}   # delist-eviction: consecutive scans a symbol is gone
        self._msgs = 0
        self._backpressured = False
        self._conn_tasks: Dict[str, asyncio.Task] = {}
        # per-connection observability (name -> {syms, up, reconnects, conn_errors}); instrumentation only,
        # populated in _connection_loop, emitted per-connection in _emit_perf. Does NOT change behavior.
        ...
        self._conn_errors = 0
        self._flushes = 0
        self._flush_ms = deque(maxlen=2000)   # recent flush (pipeline execute) latencies, ms
        self._last_perf = (time.time(), 0)    # (wall, msgs) at last perf emit
    ...
    async def proxy_config(self) -> Optional[Dict[str, Any]]:
        """SOCKS5 dict for connect_with_proxy_config, or None (direct). Proxied exchanges override."""
        return None
    ...
    async def get_symbols(self) -> List[str]:
        ...
        # Trust the feeder's published universe (the market-data handler already applied the quote-asset
        # policy); just drop meta keys like "_version". Re-filtering by suffix here would wrongly exclude
        # symbols whose quote isn't in the suffix (e.g. bybit USDC perps named <BASE>PERP).
    ...
    async def _connection_kwargs(self, proxied: bool) -> Dict[str, Any]:
        # Over SOCKS5 enable lib-level ws ping (keeps the tunnel alive); direct relies on the
        # silence detector + any plugin app-ping.
        return {
    ...
    async def _connection_loop(self, name: str, symbols: List[str]):
        ...
        while not self.shutdown.is_set():
            ...
            try:
                ...
                ws = await connect_with_proxy_config(await self.ws_url(), pc,
                                                     **(await self._connection_kwargs(pc is not None)))
                if established:
                    self._reconnects += 1   # any connect after the first on this loop = a reconnect
                ...
                if st is not None:
                    st["up"] = True
                    if established > 1:
                        st["reconnects"] += 1   # per-connection reconnect (mirrors the global counter above)
                await self.subscribe(ws, symbols)
                # No gap marker on (re)connect: this is a SNAPSHOT feed (every frame is a full top-N
                # book), so a reconnect loses no state — the next frame is current and recv_ts age
                # already signals staleness. Writing an empty book here stranded low-update symbols
                # (e.g. thin USDC pairs) at 0 depth until a frame happened to arrive.
                ...
                # intentional resync (a plugin detected an orderbook gap/drift) — reconnect+resubscribe
                # for a fresh snapshot. NOT a connectivity fault: don't bump failures/conn_errors and
                # don't arm mark-inactive (it's already counted in OB_GAPS).
                failures = 0
    ...
    async def _recv_loop(self, ws, symbols: List[str]):
        ...
        while not self.shutdown.is_set():
            try:
                ...
                continue
            last = time.time()  # ANY frame = alive (silence detector; never false-kill)
            try:
            ...
            try:
                ...
                raise  # plugin hit a gap -> reconnect + resubscribe -> fresh snapshot
            except Exception as e:
    ...
    def _on_books(self, books: List[Book]):
        """NON-BLOCKING hot path: stash latest snapshot per symbol."""
    ...
    async def _evict(self, symbols: List[str]):
        """Delist-eviction: a symbol the feeder UNIVERSE no longer lists (gone for evict_after_scans
        consecutive monitor passes) is fully removed — dropped from active AND inactive, its stream
        deleted. Distinct from _mark_inactive (a connectivity fault on a still-listed symbol, which goes
        to the inactive set so it can recover). Without this, delisted symbols pin a stale book/stream
        and stay in active_symbols forever (the engine then reads a frozen book)."""
        syms = [self.normalize_symbol(s) for s in symbols]
        try:
            pipe = self.redis.pipeline(transaction=False)
            pipe.srem(get_active_symbols_key(self.exchange, self.market_type), *syms)
    ...
    async def _reconcile_orphans(self, universe: Set[str]):
        """One-shot startup cleanup: prune active/inactive members (+ their streams) for symbols the
        feeder UNIVERSE no longer lists — e.g. delisted while NO producer was running, so the in-run
        delist-evictor never saw them leave (it only tracks symbols this process knew). Skipped on an
        empty universe (feeder not ready -> don't wipe everything)."""
        if not universe:
            return
        if self.is_distributed:
            # multi-worker: `universe` is only THIS worker's shard (get_symbols -> _filter_for_worker),
            # but the active/inactive sets hold ALL workers' symbols -> a full-set reconcile would evict
            # other workers' LIVE symbols. Skip; in-run eviction (also single-worker-gated) handles delists.
            return
        try:
        ...
        # count-collapse floor (mirrors the feeder's never-blank min_fraction guard): if the universe is
        # implausibly small vs what's already live, the feeder is degraded or cold-starting (it can write
        # any count >= 50% of its baseline, and =0 on its first cycle) -> SKIP rather than mass-evict live
        # symbols. A genuine delist is still pruned later by the consecutive-scan in-run evictor.
    ...
    async def _heartbeat_loop(self):
        while not self.shutdown.is_set():
            try:
                ...
                # (manual DEL/FLUSHDB, eviction, a consumer clearing keys, redis restart). Activation
                # is otherwise once-per-symbol, so a lost set would never rebuild — and the engine
                # reads symbols_status:{ex}:{mt}:active_symbols. Idempotent SADD; cheap.
                if self.active:
    ...
    async def _monitor_loop(self):
        ...
        while not self.shutdown.is_set():
            ...
            try:
                ...
                # delist-eviction (single-worker only — multi-worker shards make "left the universe"
                # ambiguous): a monitored symbol absent from the feeder universe AND no longer producing
                # data for evict_after CONSECUTIVE scans is delisted -> evict. TWO guards keep a still-live
                # symbol safe: (a) the data-flow check — a symbol still writing books within stale_symbol_age
                # is NOT a delist (covers a feeder hiccup that drops a streaming pair, or a brief halt);
                # (b) the consecutive counter, reset on reappearance OR fresh data, tolerates transient blips.
                if not self.is_distributed:
                    now = time.time()
                    for s in current:
                        self._absent_scans.pop(s, None)
    ...
    def _emit_perf(self):
        ...
        # per-connection observability (instrumentation only) — one line per connection + a summary, so a
        # single churning connection is visible at a glance instead of hidden in the aggregate counters.
        # SEMANTICS: `up` = the recv loop is running (connected + subscribed), NOT "data is flowing". For
        # data-flow health read `last_data_age_s`: None means NO symbol on this connection has EVER produced
        # a book (a silently-accepted-but-dead subscription, or an all-untraded batch) — distinct from a
        # healthy connection's small numeric age. So alert on age, not on `up` alone.
        try:
