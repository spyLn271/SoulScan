...
from src.DEX.DEXes.Raydium.RaydiumScheme import RaydiumCLMMDependenciesScheme, RaydiumCacheCLMMDependenciesScheme
####################################
...
1)
    Raydium CLMM is Uniswap V3 for Solana. There is no Dynamic Fee.
2) 
...
class RaydiumCLMMTranslater(Translater):
    ...
    def translate_PoolState(self, x: str) -> dict:
        try:
            parsed_data = self.translate(data=x, market='raydium', name='PoolState')
            if not parsed_data:
...
class RaydiumCLMM(Solana):
    ...
    def _get_start_index(current_tick, tick_spacing) -> int:
        TICK_ARRAY_SIZE = 60
        return (current_tick // (TICK_ARRAY_SIZE * tick_spacing)) * (TICK_ARRAY_SIZE * tick_spacing)
