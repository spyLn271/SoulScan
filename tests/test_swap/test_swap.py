...
def test_raydium_amm():
    r = redis.Redis(port=6380)

    market = 'raydium'
    version = 'amm'
    pool = '8WwcNqdZjCY5Pt7AkhupAFknV2txca9sq6YBkGzLbvdt'
    x_decimal = 9
    y_decimal = 6
    ...
    target_pool_state = RaydiumHybridAmmScheme(**current_state.get(pool))
    ...
    res_test_1 = swap.swap(params=params_test_1, dex=market, version=version)
    print(f"TEST 1: {params_test_1} \n"
          f"RESULT 1: {res_test_1} \n")
    print(f"Output: {res_test_1['result'] / 10 ** x_decimal}")
    print("_"*90)
    ...
    res_test_2 = swap.swap(params=params_test_2, dex=market, version=version)
    print(f"TEST 2: {params_test_2} \n"
          f"RESULT 2: {res_test_2} \n")
    print(f"Output: {res_test_2['result'] / 10 ** y_decimal}")
    print("_"*90)

    params_test_3 = swap.swap_params(
        pool_state=target_pool_state.model_dump(),
        metadata=target_pool_metadata.model_dump(),
        delta_amount=Decimal(str(190 * 10 ** x_decimal)),
        x_to_y=True,
        amount_specified_is_input=True
    )
    res_test_3 = swap.swap(params=params_test_3, dex=market, version=version)
    print(f"TEST 3: {params_test_3} \n"
          f"RESULT 3: {res_test_3} \n")
    print(f"Output: {res_test_3['result'] / 10 ** y_decimal}")
    print("_"*90)

    params_test_4 = swap.swap_params(
        pool_state=target_pool_state.model_dump(),
        metadata=target_pool_metadata.model_dump(),
    ...
    res_test_4 = swap.swap(params=params_test_4, dex=market, version=version)
